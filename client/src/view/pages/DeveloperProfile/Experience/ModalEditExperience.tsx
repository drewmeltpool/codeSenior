...
const ModalEditExperience: React.FC<IEditModal<any>> = ({
  data,
  isOpen,
  handleClose,
}) => {
  ...
  const del = (id: IKey) =>
    handleOpen('You will delete with item. Are you sure?', () =>
      dispatch(developerOperation.deleteExperience(id))
    );
    ...
    >
      <Accordion
        ...
        component={(data) => (
          <ExperienceForm
            data={data}
            onSubmit={(form: any) => {
              dispatch(developerOperation.updateExperience(data.id, form));
            }}
          />
        )}
      ...
      <Collapse title={<Title variant="info" title="Add new" />} icon="plus">
        <ExperienceForm
          ...
          onSubmit={(form: any) => {
            dispatch(developerOperation.createExperience(form));
          }}
        />
      </Collapse>
